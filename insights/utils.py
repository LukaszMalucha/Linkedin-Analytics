...
def sector_insights(sector):
    ...
    oldest_10 = dataset.iloc[:, [6, 2]].sort_values(by=['foundedYear'])[:10]
...
def find_company(query):
    searched_companies = Company.objects.filter(name__icontains=query).values()

    for element in searched_companies:
        for key, val in element.items():
            if key == 'specialties':
                element[key] = re.sub(',', ', ', val)
            if key == 'industries':
    ...
    searched_companies = sorted(searched_companies, key=lambda k: k['name'])

    count = len(searched_companies)

    search_dict = {"companies": searched_companies,
               "count": count}
